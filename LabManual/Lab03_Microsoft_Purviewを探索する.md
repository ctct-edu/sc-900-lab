...
6. タスク5-1～タスク5-5を繰り返し、経営企画(Management)でセグメントを作成します。
...
> [解説]
>
> 保持ラベル（Retention Labels）は、Microsoft 365 環境において\データのライフサイクル管理を実施するための重要な機能です。これにより、組織内のドキュメントやメールに対して保持（保存）や自動削除のポリシーを適用し、法規制や企業ポリシーに基づいたデータ管理を効率化します。
>
...
2. 左側のナビゲーション メニューの[保持ラベル]をクリックし、さら[＋ラベルを作成]ボタンをクリックします。
...
4. [ラベル設定の定義]は、[アイテムを無期限または特定の期間保持する]を選び、[次へ]をクリックします。

5. 保持期間の定義では[アイテム保持する]は[7年]、[以下に基づき保持期間を開始する]では[アイテムの作成日時]を選び、[次へ]をクリックします。

6. [確認と完了]では[ラベルを作成]をクリックします。
...
7. [保持ラベルが作成されました]と表示されます。[このラベルをMicrosoft365の場所に発行する]を選び、[完了]をクリックします。

8. [発行するラベルを選択]では、何も変更せず、[次へ]をクリックします。

9. [ポリシー スコープ]では、何も変更せず、[次へ]をクリックします。

10. [作成するアイテム保持ポリシーの種類を選択する]では、[静的]に変更し、[次へ]をクリックします。

11. [ラベルを公開する場所を選択する]では、何も変更せず、[次へ]をクリックします。

12. [ポリシー名]では[給与保持ポリシー]と入力し、[次へ]をクリックします。

13. [完了]では[送信]をクリックします。

    ![](./media/lab4-16.png)

14. [保持ラベルが発行されました]と表示されます。[完了]をクリックして終了です。
...
4. [財務]をクリックし、さらに[日本金融関連データ]をクリックし、画面下にある[次へ]をクリックします。
...
3. [名前]には[2024_内部調査_経理部_不正会計]と入力し、[作成]をクリックします。

4. [検索の作成]をクリックし、さらに検索名には[不正会計データ]と入力し、[作成]をクリックします。

5. [不正会計データ]では[Add sources]をクリックし、[Select All]をクリックし、[Manage]をクリックします。

   ![](./media/lab4-23.png)

6. [クライアントエラー]と表示されますが、無視して[OK]をクリックします。

7. [キーワード]には[売上帳]と入力し[Run query]をクリックします。

   ![](./media/lab4-24.png)

   > 本来はデータがあれば、検索結果が表示されますが、何もでないので演習はここまでとします。
